...
function Avatar({ provider }: { provider: Provider }) {
  ...
  return (
    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-sm font-bold shrink-0 ${color}`}>
      {initials}
    </div>
  );
}
      ...
      >
        ...
        <div className="mt-5 flex gap-3 justify-end">
          ...
          <button
            onClick={onConfirm}
            disabled={loading}
            className={`px-4 py-2 rounded-xl text-sm font-medium text-white transition cursor-pointer disabled:opacity-70 flex items-center gap-2 ${
...
export default function PendingApprovalsPage() {
  ...
  async function fetchPending() {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/providers?page=1&limit=100`);
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-2xl px-5 py-4 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-blue-700 mt-0.5">
              ...
              Rejecting marks their application as rejected — they will not appear on the platform.
            </p>
          </div>
        </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 items-start">
            {filtered.map((provider, idx) => {
              ...
              return (
                <div
                  key={provider.id}
                  className="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow overflow-hidden card-enter flex flex-col"
