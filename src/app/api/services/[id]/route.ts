...
function describeChanges(
  ...
  const addChange = (
    ...
    changes.push(
      `${label}: "${String(beforeValue ?? "-")}" -> "${String(afterValue ?? "-")}"`,
    );
  };
...
async function getAffectedUserIds(serviceId: string) {
  ...
  return userIds.map((value) => String(value)).filter(Boolean);
}
...
export const PUT = withApiHandler(async (req, { params }) => {
  ...
  if (validated.categoryId) {
    const categoryDoc = await Category.findById(validated.categoryId)
      .select("name")
      .lean();
  ...
  try {
    ...
    console.error(
      "[PUT /api/services/[id]] notification error:",
      notificationError,
    );
  }
  ...
  try {
    ...
    console.error(
      "[PUT /api/services/[id]] user notification error:",
      notificationError,
    );
  }
...
export const DELETE = withApiHandler(async (req, { params }) => {
  ...
  try {
    ...
    console.error(
      "[DELETE /api/services/[id]] user notification error:",
      notificationError,
    );
  }
